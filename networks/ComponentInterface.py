...
class ComponentInterface(VelometryComponent, ABC):
    ...
    def forward(self, x, mems) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def _forward_recurse(self, x: torch.Tensor, mems: List[torch.Tensor], n: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        mem2 = crop(mems[1], y)
        x = self.decoder_layers[n](torch.cat([mem2, y], dim=1))
        return (x, mems)